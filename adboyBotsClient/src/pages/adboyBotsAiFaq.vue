            ...
            <v-toolbar dark color="primary">
                    ...
                    <v-spacer></v-spacer>
                        ...
                        <v-btn icon @click="createDialogGroup()">
                            <v-icon dark>fas fa-layer-group</v-icon>
                        </v-btn>

                        <v-btn icon @click="createDialogFaq()">
                            <v-icon dark>fas fa-bullseye</v-icon>
                        </v-btn>
            ...
            <v-layout row wrap>
                <v-flex  xs6 >
                    <v-card light color="white" style= "height:450px; overflow-y: scroll; " class="pa-3"  >
                        ...
                        <template  v-for="(faqGroup,indexFaqGroup) in faqGroup">
                                ...
                                <v-card color="white" light class="mt-3"  :key="'index-Faq-Group'+indexFaqGroup">
                                    <v-card-title primary-title >
                                        ...
                                        <div slot="header" class="subheading"><v-icon color="#007bff" small class="pr-2" v-on:click="dialogEditGroup(faqGroup.faqGroupName,indexFaqGroup)">fas fa-edit</v-icon>Faq Group: {{faqGroup.faqGroupName}} <v-icon small color="#007bff" v-if="faqGroup.faqGroupName !=='General'" class="pl-2" v-on:click="confirmDeleteFaqGroup(indexFaqGroup)">fas fa-trash</v-icon></div>
                            ...
                            >
                                ...
                                <v-layout row wrap>
                                    ...
                                    <v-flex xs12>
                                    <v-btn class="info"  small @click="deleteDialogFaqGroup(myGroupName,indexFaqGroup)">Cancel</v-btn>
                                ...
                                <div v-if="selectedFaq == faq.faqName" :key="'index-Faq'+indexFaq">
                                    ...
                                    <v-flex  xs6 class="text-xs-left subheading pt-4" >
                                        ...
                                        <v-text-field
                                            ...
                                            :keyup=" isvalidKeyword(myKeyWord)"
                                            ></v-text-field>
 ...
 data: () => ({
  ...
  methods:
  {
      getFaqs(){
          ...
          let url ='https://localhost/api/getFaqs'; 
                axios({
                method:'get',
                url:url,
                 params: {
                fbPgId: this.fbPgId,templateName:this.templateName
                },
                headers: {'Authorization': `bearer ${this.leToken}`}
                })
                .then(response => {
                  console.log("Get Faqs..."+JSON.stringify(response.data));
                  this.$set(this,"myLoader",false);
                  this.$set(this,"templateName",response.data.templateName);
                  this.$set(this,'selectedFaq',response.data.selectedFaq);
                  this.$set(this,'selectedGroup',response.data.selectedGroup);
                  this.$set(this,'myScripts',response.data.myScripts);
                  this.$set(this,'faqs',response.data.faqs);
                  this.$set(this,'faqGroup',response.data.faqGroup);
                })
      ...
      saveFaqs()
      {
            let url ='https://localhost/api/saveFaqs'; 
            this.$set(this,"saveButtonColor","yellow");
                    axios({
                    method:'post',
                    url:url,
                    data:{facebookPageId:this.fbPgId,templateName:this.templateName,selectedFaq:this.selectedFaq,
                    selectedGroup:this.selectedGroup,myScripts:this.myScripts,
                    faqs:this.faqs,faqGroup:this.faqGroup},
                    headers: {'Authorization': `bearer ${this.leToken}`}
                    })
                     .then(response => {
                        console.log("after save faqs..."+JSON.stringify(response));
      ...
      createFaq()
      {
        ...
        for(var j=0; j<this.faqGroup.length; j++)
        {
            let newTag = {isSelected:true,faqName:this.myFaqName};
            this.faqGroup[j].groupOfFaqs.push(newFaq);
        }
      ...
      updateFaq(myFaqName,myfaqPosition)
      {
          ...
          for(var j=0; j<this.faqGroup.length; j++)
          {
                this.$set(this.faqGroup[j].groupOfFaqs[myfaqPosition],'faqName',this.faqs[myfaqPosition].faqName); 
                console.log(this.faqGroup[j].groupOfFaqs[myfaqPosition].faqName);    
          }
      ...
      confirmDeleteFaqGroup(indexFaqGroup)
      {
          ...
          {
          this.$delete(this.faqGroup, indexFaqGroup);
      ...
      validateNameGroupEdit(faqGroupName,indexFaqGroup)
      {
            ...
            for(var j=0; j<this.faqGroup.length; j++)
            {
                ...
                if(j !==indexFaqGroup)
                {
                    ...
                    {
                        console.log("faqGroupName..."+faqGroupName+" this.faqGroups[j].faqGroupName..."+this.faqGroup[j].faqGroupName);
      ...
      validateNameGroup()
      {
         this.faqGroup.forEach(faqGroup => {
      ...
      createGroup()
      {
          ...
          {
               newGroupOFFaqs.push({isSelected:true,faqName:this.faqs[i].faqName});
          }


         let newFaqGroup = {faqGroupName:this.myGroupName,panel:[true],groupOfFaqs:newGroupOFFaqs};

         this.faqGroup.push(newFaqGroup);
         this.$set(this,'dialogCreateGroup',false);
         this.$set(this,'myGroupNameIsValid',false);
         this.$set(this,'myGroupName',"");
