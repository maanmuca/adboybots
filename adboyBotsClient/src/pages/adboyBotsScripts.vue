            ...
            <v-container fixed grid-list-md text-xs-center >
                <v-toolbar dark color="primary">
                    ...
                    <v-toolbar-title class="white--text pt-2 display-1" >
                            ...
                            v-on:change="setTemplateNameCookie()"
                            ></v-select>
                    </v-toolbar-title>
                    <v-tooltip top>
                    ...
                    <v-menu open-on-hover v-if="templateSection.sectionName==sectionSelected" bottom left :key="'v-menu-type-Of-message'+indexTemplateSection">
                        ...
                        <v-list-tile>
                            <v-list-tile-title v-on:click="addMessage('gallery_template',indexTemplate,indexTemplateSection)">Gallery Template</v-list-tile-title>
                        </v-list-tile>
                        <v-list-tile>
                            <v-list-tile-title v-on:click="addRedirectTo(indexTemplate,indexTemplateSection)">Redirect to:</v-list-tile-title>
                        </v-list-tile>
                        </v-list>
                    </v-menu>
                    </template>
                                             ...
                                             <v-hover :key="'v-hover-index-Sections'+indexTemplateSection">
                                                <v-card light color="white" slot-scope="{ hover }" :class="`elevation-${hover ? 2 : 4}`" v-on:click="changePosSection(indexTemplate,indexTemplateSection)" >
                                                    ...
                                                    <v-layout row wrap class="pl-2  pr-2">
                                                        <v-flex  xs6 :style="templateSection.sectionName ==sectionSelected? 'border-color:#007bff; font-size:14px;':''" :class="templateSection.isModifiable? 'text-xs-left pt-2 pl-2':'text-xs-left pt-3 pl-2'">
                                                            ...
                                                            <span><i  :style="templateSection.sectionName ==sectionSelected ? 'font-style: normal; color:#007bff; font-size:14px;': 'font-style: normal; color:black; font-size:14px; margin-top:10px;'">{{templateSection.sectionName }}</i></span>
                                                        </v-flex>
                      ...
                      <v-card light color="white" style= "height:450px; overflow-y: scroll; overflow-x: scroll; " class="pa-3"  >
                        <template v-for="(templateSection,indexTemplateSection) in myTemplate.templateSections">  
                            <template v-if="sectionSelected == templateSection.sectionName" >    
                                ...
                                <v-container :key="'mySection-'+indexTemplateSection">
                                    <template v-for="(sectionMessage,indexSectionMessage) in templateSection.sectionMessages" >
                                       ...
                                       <v-divider class="pb-2 pt-2" v-if="sectionMessage.type=='button_template'" :key="'divider-'+indexSectionMessage"></v-divider> 
                                        ...
                                        <!--------------------------------------Gallery------------------------------------------>
                                        <v-layout v-if="sectionMessage.type=='gallery_template'"  :style="sectionMessage.categorySelected =='Services' ||sectionMessage.categorySelected =='--'? 'width: 456px;':'width: 712px;'" :key="'list-categories-'+indexSectionMessage" >
                                            <template v-if="isNotFirstOrLastPosition(indexTemplate,indexTemplateSection,indexSectionMessage)">
                                                <span class="pl-2 pr-2 pt-4" slot="activator" v-on:click="upPosition(indexTemplate,indexTemplateSection,indexSectionMessage)" ><v-icon small>fas fa-arrow-up</v-icon></span>
                                                <span class="pl-2 pr-4 pt-4" slot="activator" v-on:click="downPosition(indexTemplate,indexTemplateSection,indexSectionMessage)"  ><v-icon small>fas fa-arrow-down</v-icon></span> 
                                            </template>
                                            ...
                                            :key="'selected-category-'+indexTemplate+indexTemplateSection+indexSectionMessage"
                                            :items="listOfCategories"
                                            label="Select a category"
                                            v-model="sectionMessage.categorySelected"
                                            v-on:change="addElementsGallery(sectionMessage.categorySelected,indexTemplate,indexTemplateSection,indexSectionMessage)"
                                            ></v-select>
                                           
                                            <template    v-for="(myFilter,indexFilter) in sectionMessage.myFilters"> 
                                                <template v-if="sectionMessage.categorySelected !=='--'"> 

                                                        <v-select
                                                        :key="'attributeCategory'+indexSectionMessage+indexFilter"
                                                        :items="listOfAttributes(sectionMessage.categorySelected)"
                                                        label="Select attribute"
                                                        v-model="myFilter.attributeName"
                                                        class="ml-2 mr-2"
                                                        ></v-select>

                                                        <template v-if="myFilter.attributeName !=='--'">
                                                        <v-select
                                                        :key="'selected-attribute-value'+indexSectionMessage+indexFilter"
                                                        :items="listOfAtrributeValues(myFilter.attributeName,sectionMessage.categorySelected)"
                                                        label="Select value"
                                                        v-model="myFilter.attributeValue"
                                                        class="ml-2 mr-2"
                                                         v-on:change="setAttributeFilter(indexTemplate,indexTemplateSection,indexSectionMessage,myFilter.attributeName,sectionMessage.categorySelected,myFilter.attributeValue)"
                                                        ></v-select>
                                                        </template>
                                                </template>
                                            </template>
                                            ...
                                            <span class="pl-3 pr-2 pt-4"><v-icon small  color="#007bff" v-on:click="deleteGallery(indexTemplate,indexTemplateSection,indexSectionMessage)">fas fa-trash</v-icon></span>
                                        ...
                                        <v-layout v-if="sectionMessage.type=='gallery_template'" style="margin-bottom:15px;" :key="'index-gallery-container'+indexTemplate+indexTemplateSection+indexSectionMessage+indexCategory" >
                                            ...
                                            <template v-for="(element, indexElement) in sectionMessage.elements">
                                              
                                                <v-container class="pb-0 pl-0 " v-if="indexElement==0" :key="'first Card'+indexCategory+indexElement">
                                                
                                                    <v-layout v-if="sectionMessage.categorySelected !=='Services'">
                                                        ...
                                                        <span class="pl-2 pr-2 text-xs-center" style="margin: 1px 0 2px 0px; border-radius:5px;" >
                                                         <img :src="element.imageUrl" width="300" height="200" alt="">
                                                        </span>
                                                    </v-layout>
                                                    <v-layout v-if="sectionMessage.categorySelected =='Services'">
                                                        <span class="pl-2 pr-2 pt-3"><v-icon small  color="transparent">fas fa-arrow-up</v-icon></span>
                                                        ...
                                                        <p style="color:white;" class="title">{{element.title}}</p>
                                                        </span>
                                                    </v-layout>
                                                    <v-layout v-for="(button,indexButton) in element.buttons" :key="'first-button'+indexElement+indexButton"  >
                                                        <span class="pl-2 pr-2 pt-3"><v-icon small color="transparent">fas fa-arrow-up</v-icon></span>
                                                        <span class="pl-2 pr-4 pt-3" ><v-icon small color="transparent">fas fa-arrow-down</v-icon></span>
                                                ...
                                                <v-container class="pb-0 pl-0" v-if="indexElement>0" :key="'card'+indexCategory+indexElement">
                                                    <v-layout v-if="sectionMessage.categorySelected !=='Services'" >
                                                        <span class="pl-2 pr-2 text-xs-center" style="margin: 1px 0 2px 0px; border-radius:5px;" >
                                                         <img :src="element.imageUrl" width="300" height="200" alt="">
                                                        </span>
                                ...
                                <template v-if="templateSection.payload.length>0">
                                      
                                            
                            
                                    <v-container :key="'Redirect-To-'+indexTemplateSection">  
                                        
                                        <v-layout >  
                                            <span class="pl-5 pr-5 pt-3"><v-icon small  color="transparent">fas fa-arrow-up</v-icon></span>
                                            <span class="pl-5 pr-5 pt-3" ><v-icon small  color="transparent">fas fa-arrow-down</v-icon></span>
                                            <v-hover  :key="'v-hover-payload-'+indexTemplateSection">  
                                                <span slot-scope="{ hover }" v-on:click="dialogRedirectTo()" :class="indexQuickReply==0? `pt-3 pl-2 pr-2 mr-2 elevation-${hover ? 2 : 4}`:`pt-3 pl-2 pr-2 ml-2 mr-2 elevation-${hover ? 2 : 4}`"  style="background-color:transparent; width:200px; height:64px; border:solid 1px #007bff; margin: 1px 0 2px 0px; border-radius:5px;" >
                                                    Redirect to:<strong style="color:#007bff;">{{templateSection.payload}}</strong>
                                                </span>
                                             </v-hover>
                                            <span class="pl-3 pr-2 pt-3" contenteditable="false"><v-icon color="#007bff" v-on:click="deleteRedirectTo(indexTemplate,indexTemplateSection)"  small>fas fa-trash</v-icon></span>
                                        </v-layout>
                                    </v-container>
                                     <v-dialog
                                            v-model="dialogRedirectToControl"
                                            max-width="350"
                                            :key="'section-Dialog'+indexTemplateSection"
                                      >
                                            <v-card>
                                            ...
                                            <v-layout row wrap class="text-xs-center">
                                                <v-flex xs12>  
                                                 <p>Select a Section to Redirect</p>
                                                </v-flex>
                                                <v-flex xs12>  
                                                 <v-select
                                                    v-model="templateSection.payload"
                                                    :items="mySections"
                                                    label="Redirect to:"
                                                ></v-select>
                                                </v-flex> 
                                                <v-flex xs12>
                                                    <v-btn class="info"  small @click="dialogRedirectTo()">Close</v-btn>
                                                </v-flex>
                                            </v-layout>
                                            </v-container>
...
<script>
 ...
 methods:
  {
      setTemplateNameCookie(){
          this.$cookie.set('templateName',this.templateSelected);
      },
      dialogRedirectTo()
      {
          if(this.dialogRedirectToControl==false)
          {
              this.$set(this,'dialogRedirectToControl',true);
               this.listOFSections();
          }
          else
          {
               this.$set(this,'dialogRedirectToControl',false);
          }
      },
      deleteRedirectTo(indexTemplate,indexTemplateSection)
      {
           this.$set(this.myTemplates[indexTemplate].templateSections[indexTemplateSection],'payload','');
      },
      addRedirectTo(indexTemplate,indexTemplateSection)
      {
          this.$set(this.myTemplates[indexTemplate].templateSections[indexTemplateSection],'payload','--');
      },
      useImgAsBackground(imageUrl){
          let bgImg = `background-image: url('${imageUrl}'); width:300px; height:150px; margin: 1px 0 2px 0px; border-radius:5px;`;
          return bgImg;    
      },
      setAttributeFilter(indexTemplate,indexTemplateSection,indexSectionMessage,attributeFilter,categoryName,atttributeFilterValue)
      {
          //alert("attributeFilter..."+attributeFilter+"...atttributeFilterValue..."+atttributeFilterValue+"..."+categoryName);
          let indexCategory = this.categories.map(function(e){ return e.categoryName}).indexOf(categoryName);
         // console.log("indexCategory..."+indexCategory);
          let indexAttribute = this.categories[indexCategory].headers.map(function(e){ return e.text}).indexOf(attributeFilter);
           console.log("indexAttribute..."+indexAttribute);
           let myNewElements =[];
            for(var i=0;i<this.categories[indexCategory].myRows.length;i++)
            {
                let arrayOFObjects = this.categories[indexCategory].myRows[i];
                var key = Object.keys(arrayOFObjects)[indexAttribute-1];
                var value = arrayOFObjects[key];
                console.log("Key value every row..."+key+"..."+value);
                if(key ==attributeFilter && value ==atttributeFilterValue)
                {
                        let buttons=[];
                        buttons.push({type:'postback',title:this.categories[indexCategory].myRows[0].button,payload:this.categories[indexCategory].myRows[0].button});
                        let  newGalleryElement = {
                        title:this.categories[indexCategory].myRows[i].title,
                        subtitle:this.categories[indexCategory].myRows[i].subtitle,
                        buttons:buttons,
                        imageUrl:this.categories[indexCategory].myRows[i].imageUrl, 
                        }
                        
                        myNewElements.push(newGalleryElement);
                }
                
            }
            this.$set(this.myTemplates[indexTemplate].templateSections[indexTemplateSection].sectionMessages[indexSectionMessage],'elements',myNewElements);
      },
     
      listOfAtrributeValues(attribute,categoryName)
      {
           let indexCategory = this.categories.map(function(e){ return e.categoryName}).indexOf(categoryName);
           let pos =this.categories[indexCategory].headers.map(function(e){ return e.text}).indexOf(attribute);
           //alert("pos attribute"+pos);
           ...
           for(var i=0;i<this.categories[indexCategory].myRows.length;i++)
           {
                ...
                let arrayOFObjects = this.categories[indexCategory].myRows[i];
                //console.log("arrayOFObjects..."+ JSON.stringify(arrayOFObjects));
                var key = Object.keys(arrayOFObjects)[pos-1];
                //console.log("key.."+ key);
                var value = arrayOFObjects[key];
            ...
            let arrayOfUniqueValues = Array.from(new Set(arrayOFValues));
            return arrayOfUniqueValues;            
      },
      deleteGallery(indexTemplate,indexTemplateSection,indexSectionMessage){
         this.$delete(this.myTemplates[indexTemplate].templateSections[indexTemplateSection].sectionMessages,indexSectionMessage);
      },
      addElementsGallery(categorySelectedGallery,indexTemplate,indexTemplateSection,indexSectionMessage)
      {
          //alert("adding category elements to gallery");
          let pos = this.categories.map(function(e){ return e.categoryName}).indexOf(categorySelectedGallery);
          //console.log("this.categories[pos]..." + JSON.stringify(this.categories[pos]));
          //console.log("categorySelectedGallery..."+categorySelectedGallery+"...pos..."+pos);
          let myNewElements=[];
          let buttons=[];
          buttons.push({type:'postback',title:this.categories[pos].myRows[0].button,payload:this.categories[pos].myRows[0].button});
          for(var i=0;i<this.categories[pos].myRows.length;i++)
          {
            
            let  newGalleryElement = {
                title:this.categories[pos].myRows[i].title,
                subtitle:this.categories[pos].myRows[i].subtitle,
                buttons:buttons,
                imageUrl:this.categories[pos].myRows[i].imageUrl,
                
                }
            myNewElements.push(newGalleryElement);
          }
          //alert("myNewElements..."+JSON.stringify(myNewElements));
          //let position = this.myTemplates[indexTemplate].templateSections[indexTemplateSection].length;
          //categorySelected:categorySelectedGallery,elements:myNewElements};
          this.$set(this.myTemplates[indexTemplate].templateSections[indexTemplateSection].sectionMessages[indexSectionMessage],'categorySelected',categorySelectedGallery);
          this.$set(this.myTemplates[indexTemplate].templateSections[indexTemplateSection].sectionMessages[indexSectionMessage],'myFilters',[{attributeName: "--",attributeValue: "--",conditionFilter: "--"}]);
          this.$set(this.myTemplates[indexTemplate].templateSections[indexTemplateSection].sectionMessages[indexSectionMessage],'elements',myNewElements);
          //alert(JSON.stringify(this.myTemplates[indexTemplate].templateSections[indexTemplateSection].sectionMessages[indexSectionMessage]));
      },
      listOfAttributes(categoryName)
      {
         ...
         // console.log("this.categories..."+JSON.stringify(this.categories));
          ...
          if(indexCategory!==-1)
          {
                //console.log("this.categories[indexCategory].headers..."+JSON.stringify(this.categories[indexCategory].headers));
                //console.log("this.categories[indexCategory].headers.length..."+JSON.stringify(this.categories[indexCategory].headers.length));
                if(this.categories[indexCategory].categoryIsEditable ==true)
                {
                    for(let i=0; i<this.categories[indexCategory].headers.length;i++)
                    {
                        let valueHeaderText = this.categories[indexCategory].headers[i].text;
                        //console.log("valueHeaderText..."+valueHeaderText);
                        if(valueHeaderText =="Actions" ||valueHeaderText =="Image Url" || valueHeaderText =="Subtitle" ||valueHeaderText =="Button" ||valueHeaderText =="Title")
                        {
                        continue;
                        }
                        else
                        {
                            //alert("valueHeaderText..."+valueHeaderText);
                            arrayOfattributes.push(this.categories[indexCategory].headers[i].text);
                        }
                    }
      ...
      listOfAttributesValues(indexCategory)
      {
           let arrayOfattributesValues=[];
      },
...
// component is the parent of all the columns
      ...
      addMessage(typeOfMessage,indexTemplate,indexTemplateSection)
      {
          ...
          switch(typeOfMessage) {
            ...
            case 'gallery_template':
                //let position = this.myTemplates[indexTemplate].templateSections[indexTemplateSection].length;
                let newGalleryTemplate = {type:'gallery_template',categorySelected:"--", myFilters:[{attributeName:"--",attributeValue:"--",conditionFilter:"--"}],elements:[]};
      ...
      createTemplate()
      {
          ...
          {
            sectionName:"Default Answer",
            subtitleSection:"Default Answer when no term in known",
            isModifiable:false,
            payload:"--",
            sectionMessages:
            [
                {
                  type:'simple_text',
                  text:"This is a default answer , you can edit it!"
                }
            ]
          }]};
